export interface IWritingSubmissionItem {
  id: number;
  no: number;
  text: string;
}

const mockWritingSubmissionsData: IWritingSubmissionItem[] = [
  {
    id: 1,
    no: 1,
...
export async function getMockWritingSubmissionsData(_examId: number) {
  await new Promise(resolve => setTimeout(resolve, 1000));
  return mockWritingSubmissionsData;
}
